...
public class OracleNorthWindDatabaseInspectorIntegrationTest {
    ...
    public static final ResourceRule<Server> oracleServer = new ResourceRule<>(
            ServerFixture.server(
                    "oracle-etl-test-nw",
                    49161,
                    RdbmsScripts.startupScript("Oracle"),
    ...
    public static void setUp() throws Exception {
        ...
        url = String.format("jdbc:oracle:thin:@%s:%s:%s", oracleServer.get().ipAddress(), 49161, "XE");

        RdbmsClient system = new RdbmsClient(
                url,
                RdbmsClient.Parameters.DBUser.value(),
                RdbmsClient.Parameters.DBPassword.value());
        system.executeSkippingExceptions(RdbmsScripts.northwindStartupScript("Oracle").value());

        RdbmsClient exclusion = new RdbmsClient(
                url,
                "northwind",
                "northwind");
        exclusion.executeWithPeriodicCommit(RdbmsScripts.northwindScript("Oracle").value(), 300);
    ...
    private static void exportFromOracleToNeo4j() throws IOException {
        ...
        NeoIntegrationCli.executeMainReturnSysOut(new String[]{"export",
                        "--rdbms:url", url,
