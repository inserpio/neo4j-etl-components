...
public class MySqlExclusionIntegrationTest {
    ...
    public static void setUp() throws Exception {
        url = String.format("jdbc:mysql://%s:%s/%s?autoReconnect=true&useSSL=false", mySqlServer.get().ipAddress(), 3306, "exclusion");
        RdbmsClient client = new RdbmsClient(url);
        client.execute(RdbmsScripts.exclusionScript("MySQL").value());
